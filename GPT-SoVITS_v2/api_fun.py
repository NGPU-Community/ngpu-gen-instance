...
def handle_change(path, text, language):
    if is_empty(path, text, language):
        return JSONResponse({"code": 400, "message": "missing any of  path, text, language"}, status_code=400)
...
def handle(refer_wav_path, prompt_text, prompt_language, text, text_language, cut_punc=None, top_k=20, top_p=0.6, temperature=0.9, speed=1):
    if (
        ...
        if not default_refer.is_ready():
            return JSONResponse({"code": 400, "message": "no refer audio and not preset"}, status_code=400)
