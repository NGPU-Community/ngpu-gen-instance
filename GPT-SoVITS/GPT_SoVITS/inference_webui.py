...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free = False,speed=1,if_freeze=False,inp_refs=123):
    print(f"enter get_tts_wav 1, {ref_wav_path}, {prompt_text}, {prompt_language}, {text}, {text_language}, {dict_language}, {device}")
    global cache
    ...
    print("enter get_tts_wav 1.5")
    text_language = dict_language[text_language]
    print("enter get_tts_wav 2")
    ...
    print(f"enter get_tts_wav 2.5, ref_free={ref_free}")
    if not ref_free:
        with torch.no_grad():
            ...
            print("enter get_tts_wav 2.8")
            prompt = prompt_semantic.unsqueeze(0).to(device)

    t1 = ttime()
    t.append(t1-t0)
    print(f"enter get_tts_wav 3, {t1-t0}")
    ...
    print("enter get_tts_wav 4")
    
    ###from edward for srt file
    subs = pysrt.SubRipFile()
    for i_text,text in enumerate(texts):
        newOne = pysrt.SubRipItem()
        # 解决输入目标文本的空行导致报错的问题
        if (len(text.strip()) == 0):
            continue
        ...
        audioAll = []
        if(len(audio_opt) != 0 ):
            audioAll = np.concatenate(audio_opt, 0)
        dataInBuffer = len(audioAll)
        timeInBuffer = dataInBuffer * 1000 / hps.data.sampling_rate; #in ms
        print(f"before inferencing, data length ={dataInBuffer}, time={timeInBuffer} ms")
        hour = timeInBuffer/1000/3600
        minute = (timeInBuffer - hour * 3600 * 1000) / 1000 / 60
        second = (timeInBuffer - hour * 3600 * 1000 - minute * 60 * 1000) / 1000
        milSecond = (timeInBuffer - hour * 3600 * 1000 - minute * 60 * 1000 - second * 1000)
        startTime = pysrt.SubRipTime(hour, minute, second, milSecond)
        newOne.start = startTime
        timeStart = timeInBuffer
        ...
        print("enter get_tts_wav 4.1")
        bert = bert.to(device).unsqueeze(0)
        all_phoneme_len = torch.tensor([all_phoneme_ids.shape[-1]]).to(device)

        t2 = ttime()
        print(f"enter get_tts_wav 4.2, {t2-t0}")
        ...
        t3 = ttime()
        print(f"enter get_tts_wav 4.21, {t3-t0}")
        refers=[]
        # if(inp_refs):
        ...
        t3 = ttime()
        print(f"enter get_tts_wav 4.3, {t3-t0}")
        max_audio=np.abs(audio).max()#简单防止16bit爆音
        if max_audio>1:audio/=max_audio
        audio_opt.append(audio)
        audio_opt.append(zero_wav)
        t3 = ttime()
        print(f"enter get_tts_wav 4.31, {t3-t0}")
        audioAll = np.concatenate(audio_opt, 0)
        dataInBuffer = len(audioAll)
        timeInBuffer = dataInBuffer * 1000 / hps.data.sampling_rate; #in ms
        hour = timeInBuffer/1000/3600
        ...
        second = (timeInBuffer - hour * 3600 * 1000 - minute * 60 * 1000) / 1000
        milSecond = (timeInBuffer - hour * 3600 * 1000 - minute * 60 * 1000 - second * 1000)
        endTime = pysrt.SubRipTime(hour, minute, second, milSecond)
        newOne.end = endTime
        subs.append(newOne)
        timeEnd = timeInBuffer
        print("added one sub items, start=" + str(timeStart) + ", stop=" + str(timeEnd) + ", text=" + newOne.text)
        print(f"after inferencing, data length ={dataInBuffer}, time={timeInBuffer} ms")

        t4 = ttime()
        print(f"enter get_tts_wav 4.4, {t4-t0}")
        t.extend([t2 - t1,t3 - t2, t4 - t3])
        t1 = ttime()
    print("%.3f\t%.3f\t%.3f\t%.3f" % 
           (t[0], sum(t[1::3]), sum(t[2::3]), sum(t[3::3]))
           )
    #print(f"audio_opt={audio_opt}, s={hps.data.sampling_rate}")
    yield hps.data.sampling_rate, (np.concatenate(audio_opt, 0) * 32768).astype(np.int16), subs
    t5 = ttime()
    print(f"enter get_tts_wav 5, {t5-t0}")
